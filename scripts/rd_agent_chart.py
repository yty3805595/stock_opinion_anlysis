...
<body>
    <div class="header">
        ...
        <div class="stats">
            ...
            <div class="stat">
                <div class="stat-value">{change}</div>
    ...
    <script>
        ...
        const candleSeries = candleChart.addCandlestickSeries(: '{{ colorSchemelight', ...candleColors }});
        ...
        // 添加 RSI 基准线
        rsiChart.addLineSeries({{ color: '#ef5350', lineWidth: 1, lineStyle: 2 }}).setData(rsiData.map(d => ({{ time: d.time, value: 70 }})));
        rsiChart.addLineSeries({{ color: '#26a69a', lineWidth: 1, lineStyle: 2 }}).setData(rsiData.map(d => ({{ time: d.time, value: 30 }})));
    ...
    for i, row in df.tail(120).iterrows():  # 最近120天
        ...
        if pd.notna(row['macd']):
            macd_data.append({
                ...
                "value": float(row['macd'])  # 保留给histogram用
            })
